<x-card>
    ...
    <section class="flex xs:flex-col md:flex-row gap-3 items-center w-full">
        ...
        <section class="w-full grid items-start gap-2">
            ...
            <section class="grid xs:grid-cols-1 sm:hidden md:grid md:grid-cols-2 md:gap-3">
                <div>
                    <x-label for="first_name" content="First Name"/>
                    <x-input type="text" id="first_name" name="first_name" placeholder="First Name"  :value="old('first_name')"/>
                    <x-single-error field-name="first_name"/>
                </div>
                <div>
                    <x-label for="last_name" content="Last Name"/>
                    <x-input type="last_name" id="last_name" name="last_name" placeholder="Last Name" :value="old('last_name')" />
                    <x-single-error field-name="last_name"/>
                </div>
            </section>

            <x-label for="profile_image" content="Upload picture"/>
            <x-input type="file" id="profile_image" class="!p-0" name="profile_picture" :value="old('profile_picture')"/>
    ...
    <section>
        ...
        <x-input id="git_hub_link" type="text" name="github_repo_url" placeholder="add github link" :value="old('github_repo_url')"/>
    ...
    <section class="grid grid-cols-2 md:gap-3">
        <section>
            ...
            <x-input type="date" id="Date_Of_Birth" name="date_of_birth" placeholder="DOB" :value="old('date_of_birth')"/>
        ...
        <section>
            ...
            <x-select id="gender" name="gender">
                ...
                @foreach(\App\Enums\Gender::cases() as $gender)
                    <x-option :value="$gender->value" :content="$gender->name" :selected="old('gender') === (string)$gender->value" />
                @endforeach
            </x-select>
